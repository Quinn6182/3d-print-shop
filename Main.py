...
home_tab, upload_tab, catalog_tab, contact_tab, processing_tab = st.tabs(['Home', 'Upload', 'Catalog', 'Contact', 'Processing'])
with home_tab:
    st.subheader("Home")
        ...
        with st.form('order_data'):
            ...
            if submitted:
                ...
                with open('orders.csv', 'a') as f:
                    hash_data = str(hash(f"{color},{file_name},{amount},TBD,{name},{date},TBD\n"))
                    f.write(f"{color},{file_name},{amount},TBD,{name},{date},{hash_data},TBD\n")
                st.subheader("Submitted! 😀")
                st.write("Keep this information, this is your order number used for tracking")
                st.write(hash_data)
...
with catalog_tab:
    ...
    for i, v in enumerate(names):
        if option == v:
            ...
            if order:
                with open('orders.csv', 'a') as f:
                    hash_data = str(hash(f"{color},{option},{amount},{total},{name},{date},{size}\n"))
                    f.write(f"{color},{option},{amount},{total},{name},{date},{hash_data},{size}\n")
